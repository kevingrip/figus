    ...
    figuritas.forEach(figu => {
        ...
        botonMas.addEventListener('click', async () => {
            ...
            try {
                ...
                const respuesta = await fetch(`${api}/album/${albumRuta}/stockfiguritas/incrementar/${proveedor}/${figu._id}`, {
                    method: "PATCH"
                });
        ...
        botonMenos.addEventListener('click', async () => {
            ...
            if (figu.STOCK[proveedor].CANT > 0) {
                try {
                    //ACTUALIZAR CON MONGO
                    const respuesta = await fetch(`${api}/album/${albumRuta}/stockfiguritas/decrementar/${proveedor}/${figu._id}`, {
                        method: "PATCH"
                    });
