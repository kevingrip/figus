def renamePaises(listaOriginal,lista3LetrasPaises,variantesTotales):
    ...
    CAN = ['CAN','CANADA','CANAD','CANA']
    CMR = ['CMR','CAMERUN','CAMEROON','CAMERN','CAM','CM','CAMERON','CRM','CAME']
    CRC = ['CRC','COSTARICA','COSTA RICA','COST','ELESCUDODECOSTARICA']
    CRO = ['CRO','CROACIA','CROATIA','CROA']
    DEN = ['DEN','DINAMARCA','DENMARK','DINMICA','DENAMARK','DENMARARK']
    ECU = ['ECU','ECUADOR']
    ENG = ['ENG','INGLATERRA','ENGLAND']
    ESP = ['ESP','ESPAÑA','SPAIN','ESPAA']
    FRA = ['FRA','FRANCIA','FRANCE','FRAC']
    GER = ['GER','GERMAN','ALEMANIA','ALE','GERM']
    GHA = ['GHA','GHANA','GANA']
    IRN = ['IRN','IRAN','IRÁN','IRIRAN','LRN']
    JPN = ['JPN','JAPON','JAPAN','JAPN','JAP']
    KOR = ['KOR','KOREA','COREA','KOREADELSUR','COR']
    KSA = ['KSA','ARABIASAUDITA','ARABIA','SAUDIAR','ARAB','ARABIASAUDI']
    MEX = ['MEX','MEXICO','MEJICO','MXICO','MÉXICO','MX']
    MRR = ['MRR','MAR','MARRUECOS','MARRU']
    ...
    SUI = ['SUI','SUIZA','SUSA','SWITZERLAND','SIU']
